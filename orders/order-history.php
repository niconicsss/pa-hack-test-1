...
<?php if (count($orders) > 0): ?>
    <?php foreach ($orders as $order): ?>
        <div class="order-card">
            <h3>Order #<?= htmlspecialchars($order['id']) ?> - <?= htmlspecialchars($order['business_name']) ?></h3>
            <p><strong>Status:</strong> <?= htmlspecialchars($order['status']) ?></p>
            <p><strong>Details:</strong> <?= nl2br(htmlspecialchars($order['order_details'])) ?></p>
            <p><small>Placed on: <?= date("F j, Y, g:i a", strtotime($order['created_at'])) ?></small></p>
        </div>
    <?php endforeach; ?>
<?php else: ?>
    <p>You have not placed any orders yet.</p>
